...
```sh
db.products.insertMany([{model:"2659",brand:"Air Jordans",color:"Black"},{model:"2639",brand:"Air Jordans",color:"White"},{model:"1239",brand:"Air Jordans",color:"Red"},{model:"4059",brand:"Air Jordans",color:"Blue"}])
```

4. Update all **red** Jordans to **Maroon**:

```sh
db.products.updateMany({brand:"Air Jordans",color:"Red"},{$set:{color:"Maroon"}})
```

5. Delete all model **4281** Air Jordans.

```sh
db.products.deleteMany({model:"4281" , brand:"Air Jordans"})
...
```sh
db.products.updateMany({},{$set:{sizes:[39,40,42],qty: 10}})
```
